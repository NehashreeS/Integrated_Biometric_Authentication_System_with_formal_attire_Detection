...
def open_admin_page():
    ...
    admin_window.geometry("800x600")  # Adjust dimensions as needed
    ...
    tk.Button(left_frame, text="Remove Profile", font=("Arial", 14), bg="lightblue",
              command=lambda: display_remove_profile_form(right_frame)).pack(pady=25, padx=10, fill=tk.X)
    tk.Button(left_frame, text="Edit Profile", font=("Arial", 14), bg="lightblue").pack(pady=25, padx=10, fill=tk.X)
    tk.Button(left_frame, text="View Profiles", font=("Arial", 14), bg="lightblue").pack(pady=25, padx=10, fill=tk.X)

    tk.Button(left_frame, text="Close", font=("Arial", 12), bg="red", fg="white", command=admin_window.destroy).pack(pady=20)
...
def display_add_profile_form(right_frame):
    ...
    downloads_folder = os.path.expanduser("~/Downloads")
    tk.Label(right_frame, text="Face Image:", font=("Arial", 14), bg="lightgray").pack(pady=5)
    face_image_path = tk.StringVar()
    tk.Entry(right_frame, textvariable=face_image_path, font=("Arial", 12), state="readonly").pack(pady=5)
    ...
    tk.Button(right_frame, text="Upload Fingerprint Image", font=("Arial", 12),
              command=lambda: select_fifile(fingerprint_image_path)).pack(pady=5)

    # Add User Button
    tk.Button(right_frame, text="Add User", font=("Arial", 14), bg="green", fg="white",
              command=lambda: add_user( name_entry.get(), face_image_path.get(), fingerprint_image_path.get(),right_frame)).pack(pady=20)


def display_remove_profile_form(right_frame):
    for widget in right_frame.winfo_children():
        widget.destroy()

    tk.Label(right_frame, text="Remove Profile", font=("Arial", 18), bg="lightgray").pack(pady=10)

    tk.Label(right_frame, text="ID:", font=("Arial", 14), bg="lightgray").pack(pady=5)
    id_entry = tk.Entry(right_frame, font=("Arial", 12))
    id_entry.pack(pady=5)

    # Remove user button
    tk.Button(right_frame, text="Remove User", font=("Arial", 14), bg="green", fg="white",
              command=lambda: remove_user(id_entry.get())).pack(pady=20)
...
def save_minutiae_to_npy(FeaturesTerm, FeaturesBif, file_id, file_extension='minutiae_features.npy'):
    """
...
def select_fifile(path_variable):
    # Open file dialog to select an image
    file_path = filedialog.askopenfilename(
        filetypes=[("Image Files", "*.png;*.jpg;*.jpeg")],
        title="Select an Image"
    ...
    save_minutiae_to_npy(FeaturesTerm, FeaturesBif, file_id)


def add_user(user_id, name, face_image, fingerprint_image,right_frame):
    if not user_id or not name or not face_image or not fingerprint_image:
...
def remove_user(user_id):
    if not user_id:
        messagebox.showerror("Error", "Mention user ID")
        return
    # Add a command to remove user from the database
...
def menu_action():
    ...
    tk.Button(menu_page, text="Admin Login", font=("Arial", 14), bg="white", command=open_admin_page).pack(pady=10, anchor=tk.W, padx=20)
    ...
    if current_frame is not None:
        ...
        try:
            ...
            if person_name is not None:
                ...
                messagebox.showerror("Error", f"sorry face detection is not possible: {str(e)}")
...
def create_gui():
    ...
    # Create the label and entry for ID Number
    id_label = tk.Label(content_frame, text="ID Number", bg="white", font=("Helvetica", 20))
    id_label.grid(row=4, column=0, sticky=tk.W, padx=(0, 10), pady=(20, 10))  
    id_entry = ttk.Entry(content_frame, width=30, font=("Helvetica", 16))
    id_entry.grid(row=4, column=3, pady=(20, 10))  
    ...
    submit_button = tk.Button(main_frame, text="Submit", command=lambda: submit_image(image_entry), font=("Helvetica", 16, "bold"), bg="white", fg="black", relief="raised", width=10, height=3)
